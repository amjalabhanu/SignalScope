      ...
      <div className="flex-1 p-5 sm:p-6">
        <div className="flex flex-wrap items-start justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <h3
              className="font-['Fraunces'] text-lg leading-tight"
              style={{ color: INK }}
            >
        ...
        <aside className="lg:sticky lg:top-24 lg:h-fit">
          ...
          {entitiesLoading ? (
            <div className="space-y-1.5">
              {[...Array(4)].map((_, i) => (
                <div key={i} className="h-9 animate-pulse rounded-md" style={{ background: LINE, opacity: 0.5 }} />
              ))}
            </div>
          ) : entitiesError ? (
            <ErrorPanel message="Couldn't load your watchlist." onRetry={loadEntities} />
          ) : entities.length === 0 ? (
            <p className="px-1 text-sm" style={{ color: MUTED }}>
              No entities are being tracked yet.
            </p>
          ) : (
            <nav className="space-y-0.5">
              {entities.map((entity) => (
                <EntityRow
                  ...
                  onSelect={() =>
                    setSelectedEntityId(selectedEntityId === entity.id ? null : entity.id)
                  }
                />
              ))}
            </nav>
